...
class ChecklistScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        card = MDCard(orientation="vertical", padding=dp(16), radius=[20,20,20,20], md_bg_color=(0.9,1,0.9,1), elevation=8)
        card.add_widget(MDLabel(text="Checklist de Seguridad", font_style="H5", halign="center", theme_text_color="Primary"))
        self.checkboxes = []
        for label, icon in ITEMS:
            ...
            row.add_widget(MDLabel(text=label, font_style="Body1", halign="left", theme_text_color="Secondary"))
            card.add_widget(row)
        btn_revisar = MDRaisedButton(text="Revisar", md_bg_color=(0.2,0.7,0.2,1), on_release=self.revisar)
